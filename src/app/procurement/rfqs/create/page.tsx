...
export default function CreateRfqPage() {
  ...
  function handleSubmitRfq() {
    ...
    // Update global local storage RFQs
    try {
                          ...
                          {VENDOR_LIST.map((v) => (
                            ...
                            >
                              <input
                                ...
                                className="rounded border-zinc-800 text-cyan-500 focus:ring-cyan-500/20"
                              />
                              <span>{v}</span>
                            </label>
                          ))}
                      ...
                      {selectedVendors.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {selectedVendors.map((v) => (
                            <span
                              key={v}
                              className="inline-flex items-center gap-1 rounded-full bg-cyan-500/10 px-3 py-0.5 text-xs font-semibold text-cyan-400 border border-cyan-500/20 animate-fade-in"
                            >
                              {v}
              ...
              <div className="flex justify-between mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-900">
                ...
                {step < 5 ? (
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-600 px-6 py-3 text-xs font-bold text-white shadow-md hover:scale-[1.01] transition-all"
                  >
                    <Send className="h-4 w-4 animate-pulse" /> Broadcast Sourcing Mission
                  </button>
                )}
              </div>
            </motion.div>
